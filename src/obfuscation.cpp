    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat IQCash and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 ICS per transaction on average.
            */
            if (r <= 10) {
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeIQCashAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
bool CObfuscationPool::MakeCollateralAmounts()
{
    ...
    if (!success) {
        ...
        if (!success) {
            LogPrintf("MakeCollateralAmounts: ONLY_NOT3000IFMN Error - %s\n", strFail);
...
void CObfuscationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100ICS+1 ( bit on if present )
    // bit 1 - 10ICS+1
    // bit 2 - 1ICS+1
    // bit 3 - .1ICS+1
    // bit 3 - non-denom
...
void ThreadCheckObfuScationPool()
{
    ...
    RenameThread("iqcash-obfuscation");
