...
{
    RenameThread("iqcash-scriptch");
    scriptcheckqueue.Thread();
}

bool RecalculateICSSupply(int nHeightStart)
{
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        if (pfrom->cleanSubVer == "/IQCash Core:1.1.0/" ||
            pfrom->cleanSubVer == "/IQCash Core:1.3.0/" ||
            pfrom->cleanSubVer == "/IQCash Core:1.3.1/") {
            LOCK(cs_main);
...
{
    {
        ...
        if (!pto->fDisconnect && state.vBlocksInFlight.size() > 0 && state.vBlocksInFlight.front().nTime < nNow - 3000 * Params().TargetSpacing() * (4 + state.vBlocksInFlight.front().nValidatedQueuedBefore)) {
